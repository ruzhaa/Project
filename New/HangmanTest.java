...
public class HangmanTest {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					HangmanTest window = new HangmanTest();
	...
	private void initialize() {
		...
		exit.setBounds(1064, 496, 97, 42);
		frame.getContentPane().add(exit);

		JLabel vupros = new JLabel("tuk trqbva da prisustva vupros");
		vupros.setHorizontalAlignment(SwingConstants.CENTER);
		vupros.setForeground(new Color(0, 0, 0));
		vupros.setBackground(new Color(255, 255, 255));
		vupros.setBounds(75, 23, 428, 85);
		frame.getContentPane().add(vupros);

		JLabel answe = new JLabel(
				"tuk trqbva da sa kutiiki/ ?/ prazni mesta/ _ za populvane na bukvite");
		answe.setBackground(new Color(255, 255, 255));
		answe.setHorizontalAlignment(SwingConstants.CENTER);
		answe.setBounds(75, 230, 428, 85);
		frame.getContentPane().add(answe);
		
		JLabel pandichka = new JLabel("");
		pandichka.setHorizontalAlignment(SwingConstants.CENTER);
		Image pandaImg = new ImageIcon(this.getClass().getResource(
				"/panda_7.png")).getImage();
